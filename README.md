...
**胜利条件**：经历 3 场 Boss 战（5:00 古神现身；之后每击败一个 Boss，隔 4 分钟迎来下一场），**击败最终 Boss「终焉」即获胜**。角色死亡即失败；若 30 分钟内仍未击败终焉，也会判负（防无限拖局）。Boss 出现前 HUD 左上角会实时倒计时。
...
| 操作 | 桌面端 | 移动端 |
| --- | --- | --- |
| 移动 | `W` `A` `S` `D` / 方向键 | 屏幕底部中央的半固定虚拟摇杆：在底部操作区任意位置按下并拖动，底座常显于底部中央 |
| 攻击 | 自动 | 自动 |
| 视野缩放 | HUD 右下方 `＋` / `－`（0.5×–1.5×，中央按钮显示当前倍率并一键重置） | 同左 |
| 暂停 | `Esc` / `P` | 右下角按钮 |

**角色**：游侠 Ranger —— HP 100，移速 1.0×，起始武器「裂地印记」，固有被动「拾荒者」（拾取范围 +20%）。

**武器（基础 7 把，最多同时装备 6 把；另有 7 个进化形态替换原槽位，不额外占槽）**
...
**敌人**：5 种普通怪（蚁群、亡魂、史莱姆、幻影、漩涡虫）+ 常规精英 3 种（分裂魔、甲壳兽、欺诈者，击杀掉落 5~7 颗黄色经验宝石，不掉宝箱）+ 特殊精英「深渊炮手」（击败古神后开始周期性出现，朝玩家发射单发弹幕，同屏最多 3 只，击杀后同样掉多颗经验）+ 3 个 Boss（古神 / 灾厄 / 终焉，击败任意 Boss 掉落 5 个升级宝箱）。每种敌人有独立 AI：蓄力冲刺、持续成长、随机瞬移、螺旋接近、定期分裂、护盾减伤、受击闪现、弹幕射击。
...
| 5:00 | **Boss 1 古神** |
| 6:00 | 欺诈者进入精英轮刷，密度 ×2 |
| 击败古神 4:00 后 | **Boss 2 灾厄** |
| 击败灾厄 4:00 后 | **Boss 3 终焉（击败即通关）** |

开局 → 击败古神前普通怪生成率减半（约减 50% 让早期更从容）；击败古神后升到 70%（仍少于满额），并开始周期性刷新「深渊炮手」。后续若未通关，密度与强度继续按时间成长；30:00 未击败终焉则判负。
...
> 构建命令用 `npm install` 而非 `npm ci`：Render 会为构建保留 `node_modules` 缓存，`npm ci` 每次都会清空重装拖慢部署，而 `npm install` 能复用缓存显著提速；`package-lock.json` 仍在提交中，可复现性不受影响。仓库内同时带 `.npmrc`（关闭 audit/fund、加长网络超时与重试）提升海外构建机上的稳定性。
...
也可以手动创建：New → Static Site，Build Command 填 `npm install && npm run build`，Publish Directory 填 `dist`。

---

## 性能数据

在标题页点击「性能面板」按钮即可开启，游戏中对局内会实时显示：

| 指标 | 目标 | 实测 |
| --- | --- | --- |
| 同屏 2000+ 敌人 | ≥ 60 fps | 见性能面板 `FPS / 敌人 / draw` |
| 首屏资源体积 | < 3 MB | **dist/assets 约 647 KB 未压缩；主 bundle gzip 后约 121 KB** |
...
│   ├── render/
│   │   ├── TexKeys.ts         图集纹理索引（全部程序化绘制，无需外部素材）
...
- [ ] WebAudio 合成**音效**（攻击 / 受击 / 拾取提示音；背景音乐 BGM 已完成）
